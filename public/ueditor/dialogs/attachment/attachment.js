  ...
  UploadFile.prototype = {
    ...
    initUploader: function() {
      var _this = this,
        ...
        // WebUploader实例
        uploader,
        actionUrl = 'https://bbs.okii.com/cloudstorage/api/uploadFile', //editor.getActionUrl(editor.getOpt('fileActionName')),
      ...
      uploader = _this.uploader = WebUploader.create({
        ...
        server: actionUrl,
        fileVal: 'files', //editor.getOpt('fileFieldName'),
        duplicate: true,
      ...
      function addFile(file) {
        var $li = $('<li id="' + file.id + '">' + '<p class="title">' + file.name + '</p>' + '<p class="imgWrap"></p>' + '<p class="progress"><span></span></p>' + '</li>'),
          $btns = $(
            '<div class="file-panel">' + '<span class="cancel">' + lang.uploadDelete + '</span>' + '<span class="rotateRight">' + lang.uploadTurnRight + '</span>' + '<span class="rotateLeft">' + lang.uploadTurnLeft + '</span></div>',
          ).appendTo($li),
      ...
      function getPrefix() {
        var date = new Date();
        return date.getFullYear() + '/' + date.getMonth() + '/' + date.getDate() + '/' + date.getMinutes() + '/' + date.getSeconds() + '/' + date.getMilliseconds() + '/' + (100000 * Math.random()).toFixed(0) + '/';
      }
      uploader.on('uploadBeforeSend', function(file, data, header) {
        //这里可以通过data对象添加POST参数
        data.prefix = getPrefix();
        console.log(data);
      ...
      uploader.on('uploadSuccess', function(file, ret) {
        ...
        try {
          ...
          // 把返回的url编码一下，防止某些手机打开中文的时候报错
          var url = json.data.successList[0].url;
          var splitIndex = url.lastIndexOf('/');
          var fileName = url.slice(splitIndex + 1);
          ...
          if (json.code == '000001') {
            ...
            _this.fileList.sort(function(item1, item2) {
              return item1.id.replace('WU_FILE_', '') - item2.id.replace('WU_FILE_', '');
            });
          ...
          $file
            .find('.error')
            .text(e.message || lang.errorServerUpload)
            .show();
        }
      });
