...
(function(Highcharts, UNDEFINED) {
  ...
  var radialAxisMixin = {
    ...
    setOptions: function(userOptions) {
      this.options = merge(this.defaultOptions, this.defaultRadialOptions, userOptions);
    },
    ...
    getPosition: function(value, length) {
      ...
      return this.postTranslate(this.translate(value), pick(length, this.center[2] / 2) - this.offset);
    },
    ...
    getTitlePosition: function() {
      ...
      return {
        ...
        y: chart.plotTop + center[1] - { high: 0.5, middle: 0.25, low: 0 }[titleOptions.align] * center[2] + (titleOptions.y || 0),
      };
    },
  };
    ...
    drawPoints: function() {
      ...
      each(points, function(point) {
        ...
        if (point.plotY !== UNDEFINED) {
          ...
          medianPath = ['M', left, medianPlot, 'L', right, medianPlot, 'z'];
  ...
  seriesTypes.waterfall = extendClass(seriesTypes.column, {
    ...
    getAttribs: function() {
      ...
      var series = this,
        ...
        hoverColor = Highcharts.Color(upColor)
          .brighten(0.1)
          .get(),
  ...
  seriesTypes.bubble = extendClass(seriesTypes.scatter, {
    ...
    getRadii: function(zMin, zMax, minSize, maxSize) {
      ...
      for (i = 0, len = zData.length; i < len; i++) {
        zRange = zMax - zMin;
        pos =
          zRange > 0 // relative size, a number between 0 and 1
    ...
    drawLegendSymbol: function(legend, item) {
      ...
      item.legendSymbol = this.chart.renderer
        .circle(radius, legend.baseline - radius, radius)
        .attr({
          zIndex: 3,
        })
      ...
      each(this.series, function(series) {
        ...
        if (series.type === 'bubble' && series.visible) {
          ...
          if (isXAxis) {
            ...
            if (zData.length) {
              // #1735
              zMin = math.min(zMin, math.max(arrayMin(zData), seriesOptions.displayNegative === false ? seriesOptions.zThreshold : -Number.MAX_VALUE));
  ...
  wrap(seriesProto, 'getSegmentPath', function(proceed, segment) {
    ...
    // Connect the path
    if (this.chart.polar && this.options.connectEnds !== false && segment[segment.length - 1] === points[points.length - 1] && points[0].y !== null) {
