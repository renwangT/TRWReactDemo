            ...
            case 'A':
              while (!pp.isCommandOrEnd()) {
                ...
                var s =
                  ...
                  Math.sqrt((Math.pow(rx, 2) * Math.pow(ry, 2) - Math.pow(rx, 2) * Math.pow(currp.y, 2) - Math.pow(ry, 2) * Math.pow(currp.x, 2)) / (Math.pow(rx, 2) * Math.pow(currp.y, 2) + Math.pow(ry, 2) * Math.pow(currp.x, 2)));
                if (isNaN(s)) s = 0;
    ...
    svg.Element.tspan = function(node) {
      ...
      this.text =
        node.nodeType == 3
          ...
          ? node.childNodes[0].nodeValue // element
          : node.text;
      this.getText = function() {
        return this.text;
      };
