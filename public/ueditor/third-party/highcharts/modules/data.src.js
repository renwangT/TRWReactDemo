...
(function(Highcharts) {
  ...
  Highcharts.extend(Data.prototype, {
    ...
    parseGoogleSpreadsheet: function() {
      ...
      if (googleSpreadsheetKey) {
        jQuery.getJSON('https://spreadsheets.google.com/feeds/cells/' + googleSpreadsheetKey + '/' + (options.googleSpreadsheetWorksheet || 'od6') + '/public/values?alt=json-in-script&callback=?', function(json) {
      ...
      if (options.complete) {
        ...
        for (i = 0, seriesIndex = 0; i < columns.length; seriesIndex++) {
          ...
          for (j = 0; j < columns[i].length; j++) {
            data[j] = [firstCol[j], columns[i][j] !== undefined ? columns[i][j] : null];
            if (valueCount > 1) {
  ...
  Highcharts.wrap(Highcharts.Chart.prototype, 'init', function(proceed, userOptions, callback) {
    ...
    if (userOptions && userOptions.data) {
      Highcharts.data(
        Highcharts.extend(userOptions.data, {
