...
(function(root, factory) {
  ...
  define('lib/file', ['base', 'lib/blob'], function(Base, Blob) {
    ...
    function File(ruid, file) {
      ...
      if (!ext && this.type) {
        ext = /\/(jpg|jpeg|png|gif|bmp)$/i.exec(this.type) ? RegExp.$1.toLowerCase() : '';
        this.name += '.' + ext;
      }
      ...
      },
      {
        ...
        _nextBlock: function() {
          ...
          if (act && act.has() && act.file.getStatus() === Status.PROGRESS) {
            ...
            return isPromise(next) ? next[next.pipe ? 'pipe' : 'then'](done) : done(next);
          }
        },
  ...
  define('webuploader', ['preset/flashonly'], function(preset) {
    return preset;
  });
