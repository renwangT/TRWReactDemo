...
var CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    for (var opt in defaults) if (defaults.hasOwnProperty(opt)) options[opt] = (givenOptions && givenOptions.hasOwnProperty(opt) ? givenOptions : defaults)[opt];
    ...
    try {
      ...
      if (e.message.match(/runtime/i)) e = new Error('A CodeMirror inside a P-style element does not work in Internet Explorer. (innerHTML bug)');
      throw e;
    }
    ...
    var updateInput, userSelChange, changes, textChanged, selectionChanged, leaveInputAlone, gutterDirty, callbacks;
    ...
    var hasFocus;
    try {
      hasFocus = targetDocument.activeElement == input;
    } catch (e) {}
    ...
    var instance = (wrapper.CodeMirror = {
      ...
      setOption: function(option, value) {
        ...
        if (option == 'lineNumbers' || option == 'gutter' || option == 'firstLineNumber' || option == 'theme') operation(gutterChanged)();
      },
      ...
      addWidget: function(pos, node, scroll, vert, horiz) {
        ...
        if (vert == 'over') top = pos.y;
          ...
          if (pos.yBot + node.offsetHeight > vspace && pos.y > node.offsetHeight) top = pos.y - node.offsetHeight;
        ...
        if (scroll) scrollIntoView(left, top, left + node.offsetWidth, top + node.offsetHeight);
      },
    ...
    function setValue(code) {
      var top = { line: 0, ch: 0 };
      updateLines(top, { line: doc.size - 1, ch: getLine(doc.size - 1).text.length }, splitLines(code), top, top);
      updateInput = true;
    }
    ...
    function onMouseDown(e) {
      ...
      for (var n = e_target(e); n != wrapper; n = n.parentNode) if (n.parentNode == code && n != mover) return;

      // See if this is a click in the gutter
      for (var n = e_target(e); n != wrapper; n = n.parentNode)
        if (n.parentNode == gutterText) {
          if (options.onGutterClick) options.onGutterClick(instance, indexOf(gutterText.childNodes, n) + showingFrom, e);
      ...
      var last = start,
        going;
      if (dragAndDrop && !posEq(sel.from, sel.to) && !posLess(start, sel.from) && !posLess(sel.to, start)) {
    ...
    function onDoubleClick(e) {
      for (var n = e_target(e); n != wrapper; n = n.parentNode) if (n.parentNode == gutterText) return e_preventDefault(e);
      var start = posFromMouse(e);
      if (!start) return;
    ...
    function onKeyDown(e) {
      ...
      if (window.opera) {
        ...
        if (!handled && (mac ? e.metaKey : e.ctrlKey) && e.keyCode == 88) replaceSelection('');
      }
    }
    function onKeyPress(e) {
      if (window.opera && e.keyCode == lastStoppedKey) {
        lastStoppedKey = null;
        e_preventDefault(e);
        return;
      }
    ...
    function onFocus() {
      ...
      if (!focused) {
        ...
        focused = true;
        if (wrapper.className.search(/\bCodeMirror-focused\b/) == -1) wrapper.className += ' CodeMirror-focused';
    ...
    function unredoHelper(from, to) {
      ...
      if (change) {
        ...
        var pos = clipPos({ line: change.start + change.old.length - 1, ch: editEnd(replaced[replaced.length - 1], change.old[change.old.length - 1]) });
    ...
    function updateLinesNoUndo(from, to, newText, selFrom, selTo) {
      ...
      if (from.ch == 0 && to.ch == 0 && newText[newText.length - 1] == '') {
        ...
        for (var i = 0, e = newText.length - 1; i < e; ++i) added.push(Line.inheritMarks(newText[i], prevLine));
        if (nlines) doc.remove(from.line, nlines, callbacks);
        if (added.length) doc.insert(from.line, added);
      } else if (firstLine == lastLine) {
        if (newText.length == 1) firstLine.replace(from.ch, to.ch, newText[0]);
        else {
          lastLine = firstLine.split(to.ch, newText[newText.length - 1]);
          firstLine.replace(from.ch, null, newText[0]);
          firstLine.fixMarkEnds(lastLine);
          var added = [];
          for (var i = 1, e = newText.length - 1; i < e; ++i) added.push(Line.inheritMarks(newText[i], firstLine));
          added.push(lastLine);
    ...
    function replaceRange(code, from, to) {
      ...
      function adjustPos(pos) {
        ...
        var ch = pos.ch;
        if (pos.line == to.line) ch += code[code.length - 1].length - (to.ch - (to.line == from.line ? from.ch : 0));
    ...
    function readInput() {
      ...
      if (same < prevInput.length) sel.from = { line: sel.from.line, ch: sel.from.ch - (prevInput.length - same) };
      else if (overwrite && posEq(sel.from, sel.to)) sel.to = { line: sel.to.line, ch: Math.min(getLine(sel.to.line).text.length, sel.to.ch + (text.length - same)) };
    ...
    function visibleLines() {
      ...
      return { from: lineAtHeight(doc, from_height), to: lineAtHeight(doc, to_height) };
    }
    ...
    function updateDisplay(changes, suppressCallback) {
      ...
      var intact = changes === true ? [] : computeIntact([{ from: showingFrom, to: showingTo, domStart: 0 }], changes);
      ...
      if (lineDiv.childNodes.length != showingTo - showingFrom) throw new Error('BAD PATCH! ' + JSON.stringify(intact) + ' size=' + (showingTo - showingFrom) + ' nodes=' + lineDiv.childNodes.length);
    ...
    function computeIntact(intact, changes) {
      for (var i = 0, l = changes.length || 0; i < l; ++i) {
        ...
        for (var j = 0, l2 = intact.length; j < l2; ++j) {
          var range = intact[j];
          if (change.to <= range.from && change.diff) intact2.push({ from: range.from + diff, to: range.to + diff, domStart: range.domStart });
          else if (change.to <= range.from || change.from >= range.to) intact2.push(range);
          else {
            if (change.from > range.from) intact2.push({ from: range.from, to: change.from, domStart: range.domStart });
            if (change.to < range.to) intact2.push({ from: change.to + diff, to: range.to + diff, domStart: range.domStart + (change.to - range.from) });
          }
        }
        intact = intact2;
      }
    ...
    function patchDisplay(from, to, intact) {
      ...
      var scratch = targetDocument.createElement('div'),
        newElt;
    ...
    function updateGutter() {
      ...
      doc.iter(showingFrom, Math.max(showingTo, showingFrom + 1), function(line) {
        if (line.hidden) {
          ...
          if (marker && marker.text) text = marker.text.replace('%N%', text != null ? text : '');
    ...
    function setSelection(from, to, oldFrom, oldTo) {
      ...
      // selected range.
      if (posEq(from, to)) {
        if (!posEq(sel.from, sel.to)) changes.push({ from: oldFrom, to: oldTo + 1 });
      } else if (posEq(sel.from, sel.to)) {
        changes.push({ from: from.line, to: to.line + 1 });
      } else {
        if (!posEq(from, sel.from)) {
          if (from.line < oldFrom) changes.push({ from: from.line, to: Math.min(to.line, oldFrom) + 1 });
          else changes.push({ from: oldFrom, to: Math.min(oldTo, from.line) + 1 });
        }
        if (!posEq(to, sel.to)) {
          if (to.line < oldTo) changes.push({ from: Math.max(oldFrom, from.line), to: oldTo + 1 });
    ...
    TextMarker.prototype.clear = operation(function() {
      ...
      for (var i = 0, e = this.set.length; i < e; ++i) {
        ...
        for (var j = 0; j < mk.length; ++j) if (mk[j].set == this.set) mk.splice(j--, 1);
      }
      if (min != Infinity) changes.push({ from: min, to: max + 1 });
    });
    ...
    function markText(from, to, className) {
      ...
      if (from.line == to.line) add(from.line, from.ch, to.ch, className);
        ...
        for (var i = from.line + 1, e = to.line; i < e; ++i) add(i, null, null, className);
    ...
    function setLineHidden(handle, hidden) {
      return changeLine(handle, function(line, no) {
        if (line.hidden != hidden) {
          ...
          if (hidden && (sel.from.line == no || sel.to.line == no)) setSelection(skipHidden(sel.from, sel.from.line, sel.from.ch), skipHidden(sel.to, sel.to.line, sel.to.ch));
    ...
    function lineInfo(line) {
      ...
      var marker = line.gutterMarker;
      return { line: n, handle: line, text: line.text, markerText: marker && marker.text, markerClass: marker && marker.style, lineClass: line.className };
    }
    ...
    function measureLine(line, ch) {
      ...
      measure.innerHTML = '<pre>' + line.getHTML(null, null, false, tabText, ch) + '<span id="CodeMirror-temp-' + tempId + '">' + htmlEscape(line.text.charAt(ch) || ' ') + '</span>' + extra + '</pre>';
    ...
    function posFromMouse(e, liberal) {
      ...
      // (and !liberal).
      if (!liberal && (x - offW.left > scroller.clientWidth || y - offW.top > scroller.clientHeight)) return null;
    ...
    function onContextMenu(e) {
      ...
      if (posEq(sel.from, sel.to) || posLess(pos, sel.from) || !posLess(pos, sel.to)) operation(setCursor)(pos.line, pos.ch);
    ...
    function highlightWorker() {
      ...
      while (work.length) {
        ...
        doc.iter(i, doc.size, function(line) {
          ...
          if (compare) {
            ...
            else if (++unchanged > 3 && (!mode.indent || mode.indent(hadState, '') == mode.indent(state, ''))) return true;
          }
          ++i;
        });
        if (bail) return;
      ...
      if (foundWork && options.onHighlightComplete) options.onHighlightComplete(instance);
    }
    ...
    function endOperation() {
      ...
      if (focused && !leaveInputAlone && (updateInput === true || (updateInput !== false && selectionChanged))) resetInput(userSelChange);
      ...
      if (selectionChanged && options.onCursorActivity) options.onCursorActivity(instance);
      if (tc && options.onChange && instance) options.onChange(instance, tc);
    ...
    for (var ext in extensions) if (extensions.propertyIsEnumerable(ext) && !instance.propertyIsEnumerable(ext)) instance[ext] = extensions[ext];
    return instance;
  ...
  CodeMirror.listMIMEs = function() {
    var list = [];
    for (var m in mimeModes) if (mimeModes.propertyIsEnumerable(m)) list.push({ mime: m, mode: mimeModes[m] });
    return list;
  };
  ...
  var commands = (CodeMirror.commands = {
    ...
    transposeChars: function(cm) {
      ...
      if (cur.ch > 0 && cur.ch < line.length - 1) cm.replaceRange(line.charAt(cur.ch) + line.charAt(cur.ch - 1), { line: cur.line, ch: cur.ch - 1 }, { line: cur.line, ch: cur.ch + 1 });
    },
  ...
  MarkedText.prototype = {
    ...
    dup: function() {
      return new MarkedText(null, null, this.style, this.set);
    },
    clipTo: function(fromOpen, from, toOpen, to, diff) {
      if (this.from != null && this.from >= from) this.from = Math.max(to, this.from) + diff;
      if (this.to != null && this.to > from) this.to = to < this.to ? this.to + diff : from;
      if (fromOpen && to > this.from && (to < this.to || this.to == null)) this.from = null;
      if (toOpen && (from < this.to || this.to == null) && (from > this.from || this.from == null)) this.to = null;
    },
    isDead: function() {
  ...
  Line.prototype = {
    ...
    append: function(line) {
      ...
      if (mymk) {
        for (var i = 0; i < mymk.length; ++i) if (mymk[i].to == null) mymk[i].to = mylen;
      }
      if (mk && mk.length) {
    ...
    fixMarkStarts: function() {
      var mk = this.marked;
      if (!mk) return;
      for (var i = 0; i < mk.length; ++i) if (mk[i].from == null) mk[i].from = 0;
    },
    ...
    highlight: function(mode, state, tabSize) {
      ...
      while (!stream.eol()) {
        ...
        if (pos && st[pos - 1] == style) st[pos - 2] += substr;
        else if (substr) {
    ...
    getTokenAt: function(mode, state, ch) {
      ...
      return { start: stream.start, end: stream.pos, string: stream.current(), className: style || null, state: state };
    },
    ...
    getHTML: function(sfrom, sto, includePre, tabText, endAt) {
      var html = [],
        first = true;
      if (includePre) html.push(this.className ? '<pre class="' + this.className + '">' : '<pre>');
      ...
      if (!allText && endAt == null) span(' ', sfrom != null && sto == null ? 'CodeMirror-selected' : null);
    ...
    cleanUp: function() {
      this.parent = null;
      if (this.marked) for (var i = 0, e = this.marked.length; i < e; ++i) this.marked[i].detach(this);
    },
  };
  ...
  LeafChunk.prototype = {
    ...
    remove: function(at, n, callbacks) {
      for (var i = at, e = at + n; i < e; ++i) {
        ...
        line.cleanUp();
        if (line.handlers) for (var j = 0; j < line.handlers.length; ++j) callbacks.push(line.handlers[j]);
      }
  ...
  History.prototype = {
    addChange: function(start, added, old) {
      ...
      if (time - this.time > 400 || !last || last.start > start + added || last.start + last.added < start - last.added + last.old.length) this.done.push({ start: start, added: added, old: old });
      else {
        var oldoff = 0;
        if (start < last.start) {
          for (var i = last.start - start - 1; i >= 0; --i) last.old.unshift(old[i]);
        ...
        for (var i = last.added - oldoff, e = old.length; i < e; ++i) last.old.push(old[i]);
  ...
  function eltOffset(node, screen) {
    ...
    for (var n = node; n; n = n.offsetParent) {
      ...
      if (screen && computedStyle(n).position == 'fixed') skipBody = true;
    }
  ...
  function editEnd(from, to) {
    ...
    for (var i = from.length, j = to.length; i >= 0 && j >= 0; --i, --j) if (from.charAt(i) != to.charAt(j)) break;
    return j + 1;
  }
...
CodeMirror.defineMode('xml', function(config, parserConfig) {
  ...
  var Kludges = parserConfig.htmlMode
    ? {
        autoSelfClosers: { br: true, img: true, hr: true, link: true, input: true, meta: true, col: true, frame: true, base: true, area: true },
  ...
  function endtag(startOfLine) {
    return function(type) {
      if (type == 'selfcloseTag' || (type == 'endTag' && Kludges.autoSelfClosers.hasOwnProperty(curState.tagName.toLowerCase()))) return cont();
      if (type == 'endTag') {
...
CodeMirror.defineMode('javascript', function(config, parserConfig) {
  ...
  return {
    ...
    indent: function(state, textAfter) {
      ...
      else if (lexical.info == 'switch' && !closing) return lexical.indented + (/^(?:case|default)\b/.test(textAfter) ? indentUnit : 2 * indentUnit);
...
CodeMirror.defineMode('htmlmixed', function(config, parserConfig) {
  ...
  return {
    ...
    copyState: function(state) {
      if (state.localState) var local = CodeMirror.copyState(state.token == css ? cssMode : jsMode, state.localState);
      return { token: state.token, localState: local, mode: state.mode, htmlState: CodeMirror.copyState(htmlMode, state.htmlState) };
    },
    ...
    indent: function(state, textAfter) {
      if (state.token == html || /^\s*<\//.test(textAfter)) return htmlMode.indent(state.htmlState, textAfter);
      else if (state.token == javascript) return jsMode.indent(state.localState, textAfter);
