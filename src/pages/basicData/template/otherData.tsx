  ...
  return [
    {
      ...
      input: ({ onChange, value }: any) => {
        return <Input onChange={onChange} allowClear placeholder="请输入服务代理ID" />;
      },
    },
  ];
};
  ...
  return [
    {
      ...
      render: (text: any, record: any, index: number) => (
        <div style={{ minWidth: 28 }}>{(pagination.current - 1) * pagination.pageSize + (index + 1)}</div>
      ),
    },
    {
      title: '模板',
      ...
      render(text: any, record: any) {
        return (
          <Space>
            ...
            <Button size="small" onClick={() => remove(record.id)} type="link" danger>
              删除
            </Button>
          </Space>
        );
  ...
  return [
    {
      ...
      input: ({ onChange, value }: any) => {
        return <Input onChange={onChange} value={value} allowClear placeholder="请输入模板" />;
      },
    },
  ];
};
      ...
      input() {
        return (fields: any[], { add, remove }: any) => {
          ...
          return (
            <>
              {fields.map(field => (
                <Form.Item wrapperCol={wrapperCol} key={field.key} style={{ display: 'flex', marginBottom: 8 }}>
